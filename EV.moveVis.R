...
summary(d$id.yr)
d1 = d %>% group_by(id.yr) %>% filter(n()>=10) %>% ungroup()
...
move_data <- align_move(dm, res = 8, digit = 0, unit = "hours")
...
#customize frames
frames <- add_labels(frames, title = "Egyptian Vulture Migrations", 
...
animate_frames(frames, out_file = "./Outputs/EgyptianVulture_MovementAnimation_MigrationFlexibility_Satellite.mp4", overwrite = TRUE,
               fps = 10, end_pause = 5, res = 1000, width = 9000, height = 7000,)
